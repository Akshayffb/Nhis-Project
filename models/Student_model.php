...
class Student_model extends MY_Model
{
    ...
    public function getDatatableByClassSection($class_id = null, $section_id = null)
    {
        $this->datatables
            ...
            ->searchable('class_id,section_id,admission_no,admission_date,students.firstname,students.middlename,  students.lastname,students.father_name,students.dob,students.guardian_phone')
            ->orderable('class_id,section_id,admission_no, admission_date, students.firstname,students.father_name,students.dob,students.guardian_phone')
        ...
        if ($section_id != null) {
            ...
            // ->order_by('students.admission_date')
    ...
    public function searchdatatableByClassSectionCategoryGenderRte($class_id = null, $section_id = null
        ...
        if ($rte != null) {
         ...
         $this->datatables->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode , school_houses.house_name, vehicle_routes.route_id,vehicle_routes.vehicle_id, students.religion,     students.dob ,students.current_address,    students.permanent_address,students.category_id, categories.category, students.school_house_id, students.vehroute_id,  students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender')
        ...
        ->join('vehicle_routes', 'vehicle_routes.id = students.vehroute_id', 'left')
    ...
    public function add_student_sibling($data_sibling)
    {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->insert('student_sibling', $data_sibling+1);
            $insert_id = $this->db->insert_id();
    ...
    // adding 2nd sibling
    public function add_student_sibling1($data_sibling1)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('student_sibling', $data_sibling1);
            $message   = UPDATE_RECORD_CONSTANT . " On  student sibling id " . $data['id'];
            $action    = "Update";
            $record_id = $insert_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('student_sibling', $data_sibling1);
            $insert_id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On student sibling id " . $insert_id;
            $action    = "Insert";
            $record_id = $insert_id;
    ...
    //     return $query->result_array();
    // }
    public function searchLibraryStudent($class_id = null, $section_id = null,$searchterm=null)
    {
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,
           IFNULL(libarary_members.id,0) as `libarary_member_id`,
           IFNULL(libarary_members.library_card_no,0) as `library_card_no`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname, students.middlename, students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender')->from('students');
        ...
        if( $searchterm !=null){
          $like =" students.firstname LIKE '%".$searchterm."%' || students.lastname LIKE '%".$searchterm."%' || students.admission_no LIKE '%".$searchterm."%' || libarary_members.library_card_no LIKE '%".$searchterm."%'";
          $this->db->where($like);
        }
        $this->db->order_by('students.id');

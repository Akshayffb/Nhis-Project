...
class Book_model extends MY_Model {
    ...
    public function getbooklist($status ='') {
       ...
       $this->datatables
        ...
        if($status){
            ...
            $this->datatables->where('DATE(created_at) >= ( CURDATE() - INTERVAL 30 DAY )');
        }
    ...
    public function listcategory($id="") {
        $this->db->select()->from('book_categories');
        if($id){
            $this->db->where('id',$id);
        }
    ...
    {
        $this->db->select()->from('book_suppliers');
        if($id){
            $this->db->where('id',$id);
        }
         $this->db->where('status',1);

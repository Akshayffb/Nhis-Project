        ...
        if ($this->form_validation->run()) {
            ...
            foreach ($userlisting as $users_key => $users_value) {
                if ($users_value == "student") {
                    ...
                    $student_array = $this->db->query("SELECT s.id,s.email,s.mobileno,c.field_value as mother_email FROM `students`as s INNER JOIN custom_field_values as c ON s.id=c.belong_table_id WHERE c.custom_field_id=28")->result_array();
                ...
                if ($send_mail) {
                    if (!empty($this->mail_config)) {
                        foreach ($user_array as $user_mail_key => $user_mail_value) {
                            ...
                            if (isset($user_mail_value['mother_email']) && $user_mail_value['mother_email'] != "") {
                                // print_r($user_mail_value);exit;
                                $this->mailer->send_mail($user_mail_value['mother_email'], $message_title, $message, $_FILES);
                            }
                        }
                    }
                }

...
class Book extends Admin_Controller
{
     ...
     public function index()
    {
        ...
        $data['title_list'] = 'Book Details';
        $listbook           = $this->book_model->listbook();
        $data['categories']   =$this->book_model->listcategory();
        $data['suppliers']   =$this->book_model->listsuppliers();
        $data['listbook']   = $listbook;
        $this->load->view('layout/header');
        $this->load->view('admin/book/createbook', $data);
        $this->load->view('layout/footer');
    ...
    public function category($id="")
    {
        ...
        $list         = $this->book_model->listcategory();
        $data['list'] = $list;
        if($id){
           $data['category']         = $this->book_model->listcategory($id);  
        }
     ...
     public function addUpdateCategory($value='')
    {
            if(isset($_POST)&& !empty($_POST)){
                ...
                if(isset($_POST['id'])&& !empty($_POST['id'])){
                    $exists = $this->db->get_where('book_categories', array('category_name' => $category,'id !='=>$_POST['id']));
                    if($exists->num_rows() > 0 ){
                        $this->session->set_flashdata('msg', '<div class="alert alert-warning text-left">This category Already exists</div>');
                    }else{
                        $this->db->where('id', $_POST['id']);
                        $this->db->update('book_categories', $dataInsert);
                        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">successfully updated</div>');
                        
                    }
 
                }else{
                    $exists = $this->db->get_where('book_categories', array('category_name' => $category));
                    if($exists->num_rows() > 0 ){
                        $this->session->set_flashdata('msg', '<div class="alert alert-warning text-left">This category Already exists</div>');
                    }else{
                         $this->db->insert('book_categories',$dataInsert);
    ...
    public function barcode()
    {
        ...
        $this->session->set_userdata('top_menu', 'Library');
        $this->session->set_userdata('sub_menu', 'book/barcode');

        $data['title']      = 'Barcode';
        $data['title_list'] = 'Barcode';
        $listbook           = $this->book_model->listbook();
        $data['listbook']   = $listbook;
        $this->load->view('layout/header');
        $this->load->view('admin/book/barcode', $data);
        $this->load->view('layout/footer');
    }
        public function getAvailQuantity()
    {
        ...
        $result_final = array('status' => '1', 'qty' => $available,'book_details'=>$result);
    ...
    public function getbooklist()
    {
        ...
        if(isset($_POST[0])&& $_POST[0] =='new'){
            $listbook        = $this->book_model->getbooklist(1);
        }else{
     ...
     public function addUpdateSuppliers($value='')
    {
            if(isset($_POST)&& !empty($_POST)){
                ...
                if(isset($_POST['id'])&& !empty($_POST['id'])){
                    $exists = $this->db->get_where('book_suppliers', array('mobile' => $mobile,'id !='=>$_POST['id']));
                    if($exists->num_rows() > 0 ){
                        $this->session->set_flashdata('msg', '<div class="alert alert-warning text-left">This suppliers mobile number Already exists</div>');
                    }else{
                        $this->db->where('id', $_POST['id']);
                        $this->db->update('book_suppliers', $dataInsert);
                        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">successfully updated</div>');
                        
                    }
 
                }else{
                    $exists = $this->db->get_where('book_suppliers', array('mobile' => $mobile));
                    if($exists->num_rows() > 0 ){
                        $this->session->set_flashdata('msg', '<div class="alert alert-warning text-left">This suppliers mobile number Already exists</div>');
                    }else{
                         $this->db->insert('book_suppliers',$dataInsert);
                         $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
    ...
    public function suppliers($id='')
    {
        ...
        $list         = $this->book_model->listsuppliers();
        $data['list'] = $list;
        if($id){
           $data['category']         = $this->book_model->listsuppliers($id);  
        }
        // print_r($data['category']);exit;
        $this->session->set_userdata('top_menu', 'Library');
        $this->session->set_userdata('sub_menu', 'book/suppliers');
        $this->load->view('layout/header');
        $this->load->view('admin/book/suppliers',$data);
        $this->load->view('layout/footer');
    }
    public function deletesupplier($id='')
    {
                        $this->db->where('id', $id);
                        $this->db->update('book_suppliers',array('status' => '3'));
                        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">successfully Deleted</div>');
                         redirect('admin/book/suppliers');
    }
    public function deletecategory($id='')
    {
                        $this->db->where('id', $id);
                        $this->db->update('book_categories',array('status' => '3'));
                        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">successfully Deleted</div>');
                         redirect('admin/book/category');
    }
    // public function getbooklist()

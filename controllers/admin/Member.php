...
class Member extends Admin_Controller {
    ...
    public function bookrenew() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->bookissue_model->update($data);
            $booksNew = $this->bookissue_model->get($id);
            // print_r($_POST);
          
            $data = array(
                'book_id' => $booksNew['book_id'],
                'duereturn_date' => $date,
                'issue_date' => date('Y-m-d'),
                'member_id' => $booksNew['member_id'],
            );
              // print_r($booksNew);exit;
            $this->bookissue_model->add($data);

            $array = array('status' => 'success', 'error' => '', 'message' => "successfully renewed");
            echo json_encode($array);
        }
    }
    public function addteacherMember($value='')
    {
      $sql ="INSERT INTO libarary_members (library_card_no, member_type, member_id,is_active) SELECT CONCAT('LIB-STAFF-',s.id),'teacher',s.id,'no' FROM staff as s WHERE NOT EXISTS (SELECT l.id FROM libarary_members as l WHERE l.member_type='teacher' and l.member_id =s.id)";
      $this->db->query($sql); 
    }
    public function addstudentMember($value='')
    {
        // echo "hello";exit;
      $sql ="INSERT INTO libarary_members (library_card_no, member_type, member_id,is_active) SELECT CONCAT('LIB-SUTD-',s.id),'student',s.id,'no' FROM students as s WHERE NOT EXISTS (SELECT l.id FROM libarary_members as l WHERE l.member_type='student' and l.member_id =s.id)";
      $this->db->query($sql); 
    }
    public function index() {
        ...
        $this->addteacherMember();
        $this->addstudentMember();
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    $data['section_id'] = $this->input->post('section_id');
                    $data['search_text'] = trim($this->input->post('search_text'));
                    $resultlist = $this->student_model->searchLibraryStudent('','',$search_text);
                    $data['resultlist'] = $resultlist;
                }
            } 
            $data['sch_setting'] = $this->sch_setting_detail;

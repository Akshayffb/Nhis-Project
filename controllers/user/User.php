...
class User extends Student_Controller
{
    ...
    // try edit to for test purpose only
    public function TryStatus(){
        $id                    = $this->customlib->getStudentSessionUserID();
        $student_current_class = $this->customlib->getStudentCurrentClsSection();
        ...
        // for testing added for TryStatus
        $this->load->view('user/fees/TryStatus', $data);
    }
         ...
         public function edit()
    {
        ...
        $this->load->view('layout/student/header', $data);
            ...
            if (isset($siblings_counts) && ($total_siblings == $siblings_counts)) {
                ...
                //join to student with new parent or 
                // join the parent with the new student
                ...
                $student_sibling1 = $this->student_model->get($sibling_id);
                $update_student = array(
                    'id' => $student_id,
                    'parent_id' => $student_sibling1['parent_id'],
                );
            }

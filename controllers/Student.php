...
class Student extends Admin_Controller
{
    ...
    {
        $this->load->helper('download');
        $filepath = "./backend/import/import_student_sample_file_new.csv";
        $data     = file_get_contents($filepath);
        $name     = 'import_student_sample_file_new.csv';
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_insert = array(
                ...
                'guardian_email'    => $cus_email,
                // $this->input->post('guardian_email'),
    ...
    public function import()
    {
        ...
        $fields = array('admission_no', 'firstname', 'middlename', 'lastname', 'gender', 'dob','admission_date', 'school_house_id','email','guardian_email');
        $field =array('admission_no', 'first_name' , 'middlename' , 'last_name'  , 'gender' , 'date_of_birth',   'nationality', 'religion'  ,  'caste' ,  'sub_caste' ,  'sc_st' ,  'minority_community',  'admission_date' , 'student_house' ,  'father_name' ,'father_qualification'  ,  'father_occupation' ,  'father_company_name', 'father_designation' , 'father_office_address',   'office_number'  , 'father_aadhaar' , 'father_mobile_no'  ,  'father_email'    ,'father_annual_income'  ,  'mother_name', 'mother_qualification ' ,  'mother_ocupation'  ,  'mother_company_name' ,'mother_designation' , 'mother_office_address' ,  'mother_aadhar' ,  'mother_office_no'   , 'mother_mobile_no'  ,  'mother_email'  ,  'mother_annual_income' ,   'sats_enroll_no' , 'guardian_name' ,  'is_guardian','guardian_phone',  'guardian_relation' ,  'guardian_email' , 'guardian_occupation' ,'guardian_address'  ,  'if_permanent_address');
        $custom_fields_col =array(1=>'nationality',6=>'religion'
                                ,3=> 'caste' , 4=>   'sub_caste' , 5 => 'sc_st', 7  => 'minority_community', 8 =>'father_name',9 =>'father_qualification',10 =>'father_occupation',11=>'father_company_name' , 12=>'father_designation' ,13=>'father_office_address' , 14=>'office_number' , 15=>  'father_aadhaar', 16=> 'father_mobile_no', 17 =>'father_email' , 18 => 'father_annual_income', 19 =>'mother_name',20=>'mother_qualification', 21=>'mother_ocupation', 22 =>'mother_company_name', 23=> 'mother_designation' ,  24=>'mother_office_address',   25=>'mother_aadhar', 26=>'mother_office_no', 27=>'mother_mobile_no', 28=>'mother_email',  29 =>'mother_annual_income',30 =>'sats_enroll_no', 31=>'guardian_name',32=>'is_guardian', 34=>'guardian_phone', 35=>'guardian_relation', 36=>'guardian_email', 37=>  'guardian_occupation', 39=> 'guardian_address', 40=> 'if_permanent_address');
        $data["fields"]       = $field;
        // echo "<pre>";
        // print_r($data);
        // exit;
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            foreach ($result[$i] as $key => $value) {
                                ...
                                if (date('Y-m-d', strtotime($result[$i]['date_of_birth'])) === $result[$i]['date_of_birth'] || date('d-m-Y', strtotime($result[$i]['date_of_birth'])) === $result[$i]['date_of_birth']) {
                                    $student_data[$i]['dob'] = date('Y-m-d', strtotime($result[$i]['date_of_birth']));
                                } else {
                                    $student_data[$i]['dob'] = null;
                                }

                                if (date('Y-m-d', strtotime($result[$i]['measurement_date'])) === $result[$i]['measurement_date']|| date('d-m-Y', strtotime($result[$i]['measurement_date'])) === $result[$i]['measurement_date']) {
                                    $student_data[$i]['measurement_date'] = date('Y-m-d', strtotime($result[$i]['measurement_date']));
                                } else {
                                    $student_data[$i]['measurement_date'] = '';
                                }

                                if (date('Y-m-d', strtotime($result[$i]['measurement_date']))=== $result[$i]['admission_date'] || date('d-m-Y', strtotime($result[$i]['measurement_date']))) {
                                    $student_data[$i]['admission_date'] = date('Y-m-d', strtotime($result[$i]['admission_date']));
                                    // this line is used to convert date format 
                                } else {
                             ...
                             $cus_email='';
                               if (!empty($custom_fields_col)) {
                                    ...
                                    foreach ($custom_fields_col as $key => $value) {
                                        ...
                                        if($key==17){$cus_email=$field_value;}
                                        $custom_value_array[] = $array_custom;
                                    }
                            ...
                            $data_setting['adm_update_status'] = $this->sch_setting_detail->adm_update_status;
                            //-------------------------
                             $student_data_add =array();
                          
                            $student_data_add[$i]['firstname']=$student_data[$i]["firstname"];
                            $student_data_add[$i]['middlename']=$student_data[$i]["middlename"];
                            $student_data_add[$i]["email"] =$cus_email;
                            $student_data_add[$i]["guardian_email"] =$cus_email;
                            $student_data_add[$i]['lastname']=$student_data[$i]["lastname"];
                            $student_data_add[$i]['admission_date']=$student_data[$i]['admission_date'];
                            $student_data_add[$i]['dob']=$student_data[$i]['dob'];
                            $student_data_add[$i]['is_active'] = 'yes';
                            $student_data_add[$i]['gender'] =$student_data[$i]["gender"];
                            if ($this->sch_setting_detail->adm_auto_insert) {
                                if ($this->sch_setting_detail->adm_update_status) {
                                    $last_student                     = $this->student_model->lastRecord();
                                    $last_admission_digit             = str_replace($this->sch_setting_detail->adm_prefix, "", $last_student->admission_no);
                                    ...
                                    $student_data_add[$i]['admission_no'] =$admission_no;
                                } else {
                                    $admission_no                     = $this->sch_setting_detail->adm_prefix . $this->sch_setting_detail->adm_start_from;
                                    $student_data[$i]["admission_no"] = $admission_no;
                                      $student_data_add[$i]['admission_no'] =$admission_no;
                                ...
                                if ($admission_no_exists) {
                                    ...
                                    $insert_id = $this->student_model->add($student_data_add[$i], $data_setting);
                                }
                            } else {
  
                                if ($this->form_validation->is_unique($adm_no, 'students.admission_no')) {
                                   $student_data_add[$i]['admission_no'] = $student_data[$i]['admission_no'];
                                    $insert_id = $this->student_model->add($student_data_add[$i], $data_setting);
    ...
    public function edit($id)
    {
        ...
        // trying 1 siblings
        $siblings1                   = $this->student_model->getMySiblings($student['parent_id'], $student['id']);
        $data['siblings1'] = $siblings1;
    ...
    //                 $mother=$this->db->query("SELECT c.field_value as mother_email FROM `students`as s INNER JOIN custom_field_values as c ON s.id=c.belong_table_id WHERE c.custom_field_id=28 and s.id='".$students_value."'")->row();

    //                 if(isset($mother->mother_email)&& ! empty($mother->mother_email)){
    //                     $sender_details = array('student_id' => $students_value, 'contact_no' => $student_detail[0]->guardian_phone, 'email' => $mother->mother_email);
    ...
    //                 if(isset($mother->mother_email)&& ! empty($mother->mother_email)){
    //                     $parent_login_detail = array('id' => $students_value, 'credential_for' => 'parent', 'username' => $parent_detail->username, 'password' => $parent_detail->password, 'contact_no' => $student_detail[0]->guardian_phone, 'email' => $mother->mother_email);
    //                     $this->mailsmsconf->mailsms('login_credential', $parent_login_detail);
    //                 }
    //             }			
    //         }
            ...
            foreach ($students as $students_value) {
                ...
                if (($message_to == 1 && $notification_type == 1) || ($message_to == 1 && $notification_type == 3) || ($message_to == 3 && $notification_type == 3)) {
                    ...
                    if(isset($mother->mother_email)&& ! empty($mother->mother_email)){
                        ...
                        $sender_details = array('student_id' => $students_value, 'contact_no' => $student_detail[0]->mobileno, 'email' => $mother->mother_email);
                ...
                if (($message_to == 1 && $notification_type == 2)  || ($message_to == 1 && $notification_type == 3) || ($message_to == 3 && $notification_type == 3)) {
                    ...
                    $this->mailsmsconf->mailsms('login_credential', $student_login_detail);
                     ...
                     if(isset($mother->mother_email)&& ! empty($mother->mother_email)){
                        $student_login_detail = array('id' => $students_value, 'credential_for' => 'student', 'username' => $student_detail[0]->username, 'password' => $student_detail[0]->password, 'contact_no' => $student_detail[0]->mobileno, 'email' =>$mother->mother_email);
...
//datatable function to check search parameter validation
    ...
    public function dtstudentreportlist()
    {
        ...
        if (!empty($resultlist->data)) {
            foreach ($resultlist->data as $resultlist_key => $student) {
               ...
               // tryin to display admission date this line added here
    ...
    // array(1=>'nationality',3=>'religion' ,4=> 'caste' , 5=>   'sub_caste' , 6 => 'sc_st', 7  => 'minority_community', 8 =>'father_name' , 9 =>'father_qualification',     10 =>    'father_occupation',11=>'father_company_name' , 12=>'father_designation' ,=>13 'father_office_address' , 14=>  'office_number' , 15=>  'father_aadhaar' , 16=> 'father_mobile_no', 17 =>'father_email' , 18 => 'father_annual_income', 19 =>   'mother_name',20=>'mother_qualification', 21=>'mother_ocupation', 22 =>    'mother_company_name', 23=> 'mother_designation' ,  24=>'mother_office_address',   25=>'mother_aadhar', 26=>'mother_office_no', 27=>'mother_mobile_no', 28=>'mother_email',  29 =>'mother_annual_income',30 =>'sats_enroll_no', 31=>'guardian_name',32=>'is_guardian', 34=>'guardian_phone', 35=>'guardian_relation', 36=>'guardian_email', 37=>  'guardian_occupation', 39=> 'guardian_address', 40=> 'if_permanent_address');
}
